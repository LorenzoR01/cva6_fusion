...
class Model:
    ...
    def try_issue(self, cycle, fused, first_issued):
        ...
        if self.find_data_hazards(instr, cycle):
            can_issue = False
        if self.find_structural_hazard(instr, cycle) and not(fused == 'first'):
            can_issue = False
        self.issue_manage_last_branch(instr, cycle)
        if not self.iqlen.has(instr):
            can_issue = False
        if can_issue and not(fused == 'second' and first_issued == False):
    ...
    def run_cycle(self, cycle):
        ...
        fusion = False
        if len(self.instr_queue) > 1:
            instr0 = self.instr_queue[0].fields()
            instr1 = self.instr_queue[1].fields()
            if hasattr(instr0, 'rd') and hasattr(instr1, 'rd') and hasattr(instr0, 'rs1') and hasattr(instr0, 'rs1'):
                if (self.instr_queue[1].base() in ['LOAD', 'C.LW']) and (self.instr_queue[0].base() in ['C.ADDI', 'AUIPC']):
                    if instr0.rd == instr1.rs1 and instr0.rd == instr1.rd:
                        fusion = True
                        print("fusione!")
        if fusion == True:
            first_issued = self.try_issue(cycle, fused='first', first_issued=True)
            self.try_issue(cycle, fused='second', first_issued=first_issued)
        else:
            self.try_issue(cycle, fused='none', first_issued=True)
            self.try_issue(cycle, fused='none', first_issued=True)
        self.iqlen.fetch()
